  ...
  return (
    <main className="mx-auto max-w-3xl p-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={async ()=>{ await markAllAsRead(user.id); setItems(it=>it.map(n=> ({...n, read_at: n.read_at || new Date().toISOString()}))) }} className="rounded-md border px-3 py-1.5 text-sm text-green-700 hover:bg-green-50">Marcar todas como leídas</button>
      </div>
          ...
          <div key={n.id} className={`flex items-start gap-3 p-4 ${!n.read_at ? 'bg-orange-50/40' : ''}`}>
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-2 flex gap-2">
                ...
                {!n.read_at && <button onClick={async ()=>{ await markAsRead(n.id); setItems(it=>it.map(x=> x.id===n.id? {...x, read_at: new Date().toISOString()} : x)) }} className="rounded-md border px-2 py-1 text-xs text-gray-700 hover:bg-gray-50">Marcar leída</button>}
                <button onClick={async ()=>{ await deleteNotification(n.id); setItems(it=>it.filter(x=>x.id!==n.id)) }} className="rounded-md border px-2 py-1 text-xs text-gray-700 hover:bg-gray-50">Eliminar</button>
              </div>
            </div>
          </div>
        ))}
