...
export default function Login() {
  ...
  const redirectAbs = useMemo(() => `${location.origin}${nextParam.startsWith('/') ? nextParam : '/simple'}`, [nextParam])
  ...
  return (
    <main className="mx-auto max-w-md px-4 py-12">
      ...
      {intent === 'publish' && (
        <div className="mx-6 mt-4 rounded-lg border border-amber-200 bg-amber-50 px-4 py-2 text-sm text-amber-800">Inicia sesión para publicar tu producto. Te llevaremos a la página de publicación al entrar.</div>
      )}
  <form onSubmit={onSubmit} className="p-6 md:p-8 space-y-5">
        ...
        <div className="flex items-center justify-between">
          <label className="flex items-center gap-2 text-sm text-gray-700">
            <input type="checkbox" checked={remember} onChange={(e)=>setRemember(e.target.checked)} className="h-4 w-4 rounded border-gray-300 text-green-600 focus:ring-green-600"/>
            Recordar sesión
          </label>
          <a className="text-sm font-medium text-green-700 hover:text-green-800 hover:underline" href="/forgot-password">¿Olvidaste tu contraseña?</a>
        </div>
      ...
      <div className="px-6 pb-6 md:px-8">
        <div className="relative">
          ...
          <div className="relative flex justify-center"><span className="bg-white px-3 text-sm text-gray-500">o continúa con</span></div>
        </div>
