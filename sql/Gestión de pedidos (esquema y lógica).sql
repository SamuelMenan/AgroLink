...
create or replace function public.ensure_orders_fks() returns void
language plpgsql
as '
begin
  if not exists (select 1 from pg_constraint where conname = ''orders_product_fk'') then
    alter table public.orders
      add constraint orders_product_fk foreign key (product_id)
      references public.products(id) on delete restrict;
  end if;
  if not exists (select 1 from pg_constraint where conname = ''orders_seller_fk'') then
    alter table public.orders
      add constraint orders_seller_fk foreign key (seller_id)
      references auth.users(id) on delete cascade;
  end if;
  if not exists (select 1 from pg_constraint where conname = ''orders_buyer_fk'') then
    alter table public.orders
      add constraint orders_buyer_fk foreign key (buyer_id)
      references auth.users(id) on delete cascade;
  end if;
end;
';

select public.ensure_orders_fks();
